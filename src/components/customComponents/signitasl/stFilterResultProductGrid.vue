    ...
    <div :style="productSectionStyle">
        <div id="displayProducts" class="row well justify-center relative-position" style="min-height: 5rem;">
            ...
            <div class="col-12 lt-lg" v-if="tagsExistForScope && showTagSidebar" style="padding: 1rem .5rem 1rem .5rem;">
                <q-btn flat class="full-width bg-grey-4" icon="fa-filter" @click="showMobileFilters = !showMobileFilters" :style="showMobileFilterButtonColor">
                    Tags Filters
            ...
            <div :class="tagsExistForScope && showTagSidebar && windowWidth > 991 ? 'col-9' : 'col-12'">
                ...
                <div id="displayProducts" v-if="showDisplayProducts" class="relative-position" style="min-height: 5rem;">
                    <div class="row items-stretch justify-center">
                        <div v-if="displayProducts && displayProducts.length" :class="showDisplayProductsColSize" v-for="product in displayProducts" :key="product.productID" style="padding: .5rem;">
                            <div :is="productCardComponent" class="productCard" :componentData="product"></div>
                        </div>

                        <div :class="showDisplayProductsColSize" style="padding: .5rem;">
                            <div @click="goToDictionary()" class="q-pa-md shadow-1 cardStyle animate-fade" align="center">
                                <q-icon name="fas fa-book" style="font-size: 4rem; margin: 1rem;" />

                                <div :style="{ 'font-size': windowWidth < 1200 ? `${ adaptiveFontSize(0.9) }rem` : '1.5rem', 'padding': '.5rem', 'word-break': 'break-word', 'font-weight': 'bold' }">
                                    Dictionary
                                </div>
                                <div style="padding: .5rem; word-break: break-word;" class="text-grey-7">
                                    Sign Reference
                                </div>
                            </div>
                        </div>
...
<script>
    ...
    import filterGridSearch from '../../common/showcase/filterGridSearch'
    ...
    export default {
        ...
        mixins: [userAndProductInfo, contentItemFiltering, productCardLoad, globalComputedData, purchaseAndActivation, designTools],
        ...
        computed: {
            ...
            userActiveProducts() {
                return this.environmentProducts ? this.environmentProducts.filter((product) => {
                    // TODO: have to show signit products for testing before adding 'course' type to live products
                    return product.userActiveProduct && (product.webplayer || product.productType === 'software')
                }) : ''
            },
        ...
        methods: {
            ...
            goToDictionary() {
                if (this.userHasAccess([138345])) {
                    this.route('/play/138345/dictionary')
                } else {
